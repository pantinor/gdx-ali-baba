...
public class TmxMapRenderer extends BatchTiledMapRenderer {
    ...
    public TmxMapRenderer(GameScreen screen, TiledMap tiledMap, int[][][] roomIds, float unitScale) {
    ...
    public boolean shouldRenderCell(int roomId, int x, int y) {
        if (this.roomIds == null || roomId <= 0) {
            return true;
        }
        if (this.roomIds[x][y][0] == roomId || this.roomIds[x][y][1] == roomId || this.roomIds[x][y][2] == roomId) {
            return true;
        }
        return false;
    }
    ...
    public void renderTileLayer(TiledMapTileLayer layer) {
        ...
        for (int row = row2; row >= row1; row--) {
            ...
            for (int col = col1; col < col2; col++) {
                ...
                if (cell == null || !shouldRenderCell(this.screen.currentRoomId(), col, layerHeight - row - 1)) {
                    x += layerTileWidth;
                    continue;
                }
